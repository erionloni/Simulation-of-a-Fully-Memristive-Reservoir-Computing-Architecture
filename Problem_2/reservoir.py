...
out_dir_1 = './Problem_2/FOM Outputs/Reservoir/Conductance Evolution/'  # Directory for reservoir conductance evolution plots
out_dir_2 = './Problem_2/FOM Outputs/Reservoir/Reservoir Final states/'  # Directory for reservoir final state plots
out_dir_3 = './Problem_2/FOM Outputs/Reservoir/Voltage Outputs/'  # Directory for reservoir output voltage plots
...
# Filter the dataset to include only defined digits
indices = [np.where(y_train == i)[0][0] for i in range(4)]  # Adjust to for more digits (up to 9)
...
# Convert the grayscale images to binary images using the threshold
x_train_binary = (x_train > threshold).astype(int)

# Digit dimensions
digit_rows = digit_cols = 28

# Training patterns and their inverses
digit_list_train = [x_train_binary[i] for i in range(len(x_train_binary))]
digit_list_train_inv = [1 - np.array(digit) for digit in digit_list_train]
...
V_in_list = [[np.zeros((num_rows, timesteps_write))] for _ in range(len(digit_list_train))]
V_in_list_inv = [[np.zeros((num_cols, timesteps_write))] for _ in range(len(digit_list_train))]

# Convert pattern into pulse sequences 
train_pulses = [dataset_to_pulse_FOM(digit_rows, digit_cols, pulse_timesteps, idle_timesteps, digit_list_train, i) for i in range(len(digit_list_train))]
train_pulses_inv = [dataset_to_pulse_FOM(digit_rows, digit_cols, pulse_timesteps, idle_timesteps, digit_list_train_inv, i) for i in range(len(digit_list_train))]

# WL and BL signals
for digit in range(len(digit_list_train)):
...
currents_by_pattern = {pattern: [] for pattern in range(len(digit_list_train))}
conductances_by_pattern = {pattern: [] for pattern in range(len(digit_list_train))}
currents_at_columns_by_pattern = []

# Array to store conductance evolution
conductance_evolution = [[] for _ in range(len(digit_list_train))]

#%% RESERVOIR SIMULATION

# Loop through each pattern
for digit in range(len(digit_list_train)):
    print(f'Simulating pattern {digit}')
    ...
    for t in range(1, timesteps_write):
        ...
        if digit == 0 and t in int_point:
            filename_prefix = f'pattern_{digit}_t{int_point.index(t)}'
    ...
    plot_directed_graph(G, num_rows, num_cols, pulse_amplitude, g_min, g_max, out_dir_2, f'pattern_{digit}_final')
    plot_conductance_matrix(G, num_rows, num_cols, pulse_amplitude, g_min, g_max, out_dir_2, f'pattern_{digit}_final', g_min, g_max)

    # Plot final state of first pattern (digit == 0); adjust for other patterns
    if digit == 0:
        plot_directed_graph(G, num_rows, num_cols, pulse_amplitude, g_min, g_max, out_dir_1, f'pattern_{digit}_t5')
        plot_conductance_matrix(G, num_rows, num_cols, pulse_amplitude, g_min, g_max, out_dir_1, f'pattern_{digit}_t_int5', g_min, g_max)
...
pattern_labels = np.repeat(y_train, len(output_voltage[0]))

# Bar plot of output voltages
plot_output_voltages_FOM(output_voltage, num_cols, y_train, out_dir_3)
