...
from COM_reservoir import organized_output_voltage
from tensorflow import keras
from keras import models, layers, optimizers, losses, Input, constraints
from sklearn.metrics import confusion_matrix

# Combine voltages for each pattern into a single array
output_voltage = []
for pattern in organized_output_voltage.keys():
    combined_voltages = []
    for read_point in organized_output_voltage[pattern].keys():
        combined_voltages.extend(organized_output_voltage[pattern][read_point])
    output_voltage.append(combined_voltages)
output_voltage = np.array(output_voltage)

V_bias = 0.5 # Bias voltage
num_output_voltages = output_voltage.shape[1]  # Number of reservoir output voltages per pattern

# Output directories for different figures
out_dir_1 = './Problem_1/COM Outputs/Readout Layer/Training/'  # Directory for training-related plots
out_dir_2 = './Problem_1/COM Outputs/Readout Layer/Weights/'  # Directory for weight-related plots
out_dir_3 = './Problem_1/COM Outputs/Readout Layer/Testing/'  # Directory for testing-related plots
...
def run_single_simulation(standardize=True, save_model=False, include_bias=True):
    ...
    # Generate datasets
    train_datasets = generate_datasets(output_voltage, num_train_datasets)  # Different for every single simulation
    test_datasets = generate_datasets(output_voltage, num_test_datasets)  # Different for every single simulation
    ...
    input_dim = train_datasets_with_bias.shape[1]  # Dynamically set the input dimension (includes increased voltages)
    model = models.Sequential()
    model.add(Input(shape=(input_dim,)))  # Specify the input shape dynamically
    model.add(layers.Dense(num_classes, activation='softmax', use_bias=False, kernel_constraint=constraints.NonNeg()))  # Set the number of output nodes to num_classes
...
def run_multiple_simulations(num_runs, standardize=True, include_bias=True):
    ...
    - pattern_accuracies_evo_all_runs (list): Accuracy evolution per pattern across alln runs.
    """
    if standardize: 
